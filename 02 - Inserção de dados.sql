...
SELECT @CONTADOR = 1, @QUANTIDADE = 1001; -- *COM O SELECT É POSSÍVEL ALIMENTAR MAIS DE UMA VARIÁVEL

WHILE @CONTADOR <= @QUANTIDADE
BEGIN

	INSERT INTO CIDADES
	SELECT 'CIDADE'+CAST(@CONTADOR AS VARCHAR), (@CONTADOR%27)+1;  --(MOD(@CONTADOR,27))+1; * NO CAST É IMPORTANTE DECLARAR O TAMANHO 

	SET @CONTADOR+=1; -- *O SET SÓ ACEITA UMA VARIÁVEL
END;
GO
...
BEGIN
	...
	SELECT 'LOGRADOURO'+CAST(@CONTADOR AS VARCHAR), (@CONTADOR%10000)+1,(100000-@CONTADOR) -- * CONVERSÃO IMPLICITA 
...
DECLARE @VAR INT = 5
SELECT TOP (@VAR) WITH TIES IDVENDA, VALOR, NUMERO_ITEM -- * WITH TIES IGNORA CASO HAJA EMPATE E TRÁS TODOS
FROM ITENS_VENDAS
ORDER BY 1 DESC;
GO