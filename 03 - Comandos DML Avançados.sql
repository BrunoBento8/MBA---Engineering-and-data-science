...
/*FAÇO O MESMO DO ITEM ANTERIOR, MAS EM CASO DE EMPATE DE DOIS ITENS_VENDAS DE MESMA 
QUANTIDADE, DESEMPATE LISTANDO O ITEM DE MENOR VALOR*/
SELECT TOP 1 *
FROM ITENS_VENDAS
...
SELECT 
	LEFT(@VAR,4) AS [LEFT]
	, RIGHT(@VAR,4) AS [RIGHT]
	, LEN(@VAR) AS LEN -- * O LEN IGNORA OS ESPAÇO A DIREITA
	, CONCAT(@VAR,NULL,@VAR) AS CONCAT
	, LTRIM(@VAR) AS LTRIM -- * A FUNÇÃO TRIM LIMPA OS ESPAÇOS OS EM BRANCO, NESTE CASO O LTIM OS DA ESQUERDA
	, RTRIM(@VAR) AS RTRIM
	, LEN(TRIM(@VAR))
	, SUBSTRING(@VAR, 3, 4) AS SUBSTRING
	, LOWER(@VAR) AS LOWER
	, UPPER('aaaa') AS UPPER
	, QUOTENAME(@VAR,'"') AS QUOTENAME1
	, QUOTENAME(@VAR,'''') AS QUOTENAME2;
